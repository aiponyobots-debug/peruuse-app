...
async function fetchEmailProducts(accessToken, messageId) {
  ...
  // Prefer plain text; if not available, strip HTML tags for cleaner input
  const strippedHtml = html
    ...
    .replace(/\s{2,}/g, ' ')
    .trim();
  const textForLLM = plain || strippedHtml;

  // Extract images from HTML separately — Claude can't see images
  const imageUrl = html ? findBestImageInSlice(html) : null;
  ...
  if (llmProducts.length > 0) {
    // Attach the best image found to the first product
    if (imageUrl && llmProducts[0]) llmProducts[0].imageUrl = imageUrl;
    return llmProducts;
  }
