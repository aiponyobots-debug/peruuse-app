...
const ORDER_QUERY = '(subject:confirmation OR subject:confirmed OR subject:receipt OR subject:"order info") newer_than:180d';
...
export async function fetchOrderEmails(accessToken) {
  ...
  const messages = json.messages.slice(0, 30);
  console.log('[gmail] processing', messages.length, 'messages');

  // Process in batches of 8 to avoid rate limits on Claude
  const BATCH_SIZE = 8;
  const allProducts = [];
  for (let i = 0; i < messages.length; i += BATCH_SIZE) {
    const batch = messages.slice(i, i + BATCH_SIZE);
    const results = await Promise.allSettled(
      batch.map(m => fetchEmailProducts(accessToken, m.id))
    );
...
async function fetchEmailProducts(accessToken, messageId) {
  ...
  if (EXCLUDED_DOMAINS.some(d => fromLower.includes(d))) {
    console.log('[gmail] excluded by domain:', from);
    return [];
  }
  ...
  return [];
    ...
    console.log('[gmail] error processing message', messageId, e.message);
    return [];
  }
}
